...
static void draw_top(lv_obj_t *widget, lv_color_t cbuf[], const struct status_state *state) {
    ...
    // Battery row: left half = central, right half = peripheral.
    // Charging signaled by the inverted-bg banner. Number is shown even during
    // charge (voltage-based reading is inflated but user prefers seeing it).
    if (state->charging) {
    ...
    if (!state->peripheral_connected) {
        ...
        init_label_dsc(&pbatt_label,
                       state->peripheral_charging ? LVGL_BACKGROUND : LVGL_FOREGROUND,
                       &lv_font_unscii_8, LV_TEXT_ALIGN_CENTER);
        char right_batt[4];
